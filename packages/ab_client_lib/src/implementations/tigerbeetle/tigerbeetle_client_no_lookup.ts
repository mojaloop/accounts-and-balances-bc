...
export class TigerBeetleDataPlaneClient_NoLookup extends TigerBeetleDataPlaneClient {
    ...
    protected override async _getTbTransfersFor_checkLiquidAndReserve(req:IAnbHighLevelRequest, tmpAccounts:TB.Account[]):Promise<{tbTransfs:TB.Transfer[], success:boolean, errorMsg?:string}>{
        ...
        /* Create the 4 linked transfers
            payerControlAccountId -> hubTmpControlAccountId - pending + linked (test transf - will fail if liq check fails)
            hubTmpControlAccountId -> payerControlAccountId - pending + linked (revert test transf if successful)
            payerPositionAccountId -> hubJokeAccountId - pending + linked (reservation)
            payerControlAccountId -> hubTmpControlAccountId - pending (reflect reservation in ctrl acc)
        */
