...
import {CoaGrpcServer} from "./grpc_server/grpc_server";
...
const TIGERBEETLE_CLUSTER_ID = (process.env["TIGERBEETLE_CLUSTER_ID"]!=undefined && parseInt(process.env["TIGERBEETLE_CLUSTER_ID"])) || 0;
const TIGERBEETLE_CLUSTER_REPLICA_ADDRESSES = process.env["TIGERBEETLE_CLUSTER_REPLICA_ADDRESSES"] || "localhost:13000";
...
export class ChartOfAccountsGrpcService {
    ...
    private static grpcServer: CoaGrpcServer;
    ...
    static async start(
        ...
        // Repo.
        if (!chartOfAccountsRepo) {
            chartOfAccountsRepo = new ChartOfAccountsMongoRepo(
                this.logger,
                MONGO_URL,
                REDIS_HOST,
                REDIS_PORT
            ...
            try {
                await chartOfAccountsRepo.init();
            } catch (error: unknown) {
        ...
        if (!ledgerAdapter) {
            if(USE_TIGERBEETLE) {
                ledgerAdapter = new TigerBeetleLedgerAdapter(
                    Number(TIGERBEETLE_CLUSTER_ID),
                    [TIGERBEETLE_CLUSTER_REPLICA_ADDRESSES],
                    this.configClient.globalConfigs.getCurrencies(), logger
                );
                ...
                ledgerAdapter = new BuiltinLedgerAdapter(BUILTIN_LEDGER_SVC_URL, loginHelper, this.logger);
                //don't init
                // TODO this adapter must come from the grpcControlPlane now that the builtinLedger connects to the coa
            }
        }
        ...
        let certFiles: IAnbGrpcCertificatesFiles | undefined;
        if(USE_TLS_GRPC) {
        ...
        // gRPC server.
        this.grpcServer = new CoaGrpcServer(
            this.logger,
            this.tokenHelper,
            this.metrics,
            accountsAndBalancesAggregate,
            ACCOUNTS_AND_BALANCES_URL,
            USE_TIGERBEETLE ? "TigerBeetle" : "Builtin",
            USE_TIGERBEETLE ? TIGERBEETLE_CLUSTER_REPLICA_ADDRESSES : undefined,
            USE_TIGERBEETLE ? TIGERBEETLE_CLUSTER_ID : undefined,
            certFiles
        );
