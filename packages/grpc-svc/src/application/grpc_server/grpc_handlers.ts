...
export class GrpcHandlers {
    ...
    private async _processHighLevelBatch(
        ...
        console.log(`GrpcHandlers _processHighLevelBatch() start with batch of: ${grpcReq.requestArray.length} - peer: ${call.getPeer()}`);
        ...
        const queueItemCallback = (error: Error|null, highLevelResponses?: IAccountsBalancesHighLevelResponse[])=>{
            ...
            // TODO remove console.log()
            console.log(`GrpcHandlers _processHighLevelBatch() completed with batch of: \t${grpcReq.requestArray!.length} - peer: ${call.getPeer()}`);
