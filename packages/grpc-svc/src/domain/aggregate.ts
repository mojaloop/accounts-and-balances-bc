...
export class AccountsAndBalancesAggregate {
	...
	async createAccounts(accounts: Account[]): Promise<string[]> {
		...
		const ledgerAdapterAccounts: LedgerAdapterAccount[] = accounts.map((account) => {
			const ledgerAdapterAccount: LedgerAdapterAccount = {
				id: null, // Automatically generated by the ledger. TODO: is this correct?
				state: account.state,
				type: account.type,
		...
		const externalAccountIds: string[] = await this.ledgerAdapter.createAccounts(ledgerAdapterAccounts);

		const coaAccounts: CoaAccount[] = [];
		for (let i = 0; i < accounts.length; i++) {
			const coaAccount: CoaAccount = {
				internalId: accounts[i].id ?? externalAccountIds[i], // TODO: does this make sense? ?? or ||?
				externalId: externalAccountIds[i],
				ownerId: accounts[i].ownerId,
				state: accounts[i].state,
		...
		const retInternalAccountIds: string[] = coaAccounts.map((coaAccount) => {
			return coaAccount.internalId;
		});
		return retInternalAccountIds;
	}
	...
	async createJournalEntries(journalEntries: JournalEntry[]): Promise<string[]> {
		...
		// TODO: is there supposed to be a distinction between internal and external journal entry ids?
	...
	async getJournalEntriesByAccountId(accountId: string): Promise<JournalEntry[]> {
		...
		const journalEntries: JournalEntry[] = ledgerAdapterJournalEntries.map((ledgerAdapterJournalEntry) => {
			const journalEntry: JournalEntry = { // TODO: everything extracted from ledgerAdapterJournalEntry?
				...
				ownerId: "", // TODO: where to get the owner id from?
	...
	private async getAccountsByExternalIdsOfCoaAccounts(coaAccounts: CoaAccount[]): Promise<Account[]> {
		...
		const accounts: Account[] = ledgerAdapterAccounts.map((ledgerAdapterAccount) => {
			...
			const account: Account = {
				...
				timestampLastJournalEntry: coaAccount.timestampLastJournalEntry // TODO: should this property be part of CoaAccount?
			};
			return account;
		});
		return accounts;
