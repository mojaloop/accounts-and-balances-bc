...
import {ExpressRestServer} from "./rest_server/express_rest_server";
...
const SERVICE_NAME: string = "rest-svc";
...
// Accounts and Balances REST Service.
const ACCOUNTS_AND_BALANCES_REST_SERVICE_HOST: string =
	process.env["ACCOUNTS_AND_BALANCES_BC_REST_SERVICE_HOST"] ?? "localhost";
const ACCOUNTS_AND_BALANCES_REST_SERVICE_PORT_NO: number =
	parseInt(process.env["ACCOUNTS_AND_BALANCES_BC_REST_SERVICE_PORT_NO"] ?? "") || 1234;
...
export class RestService {
	...
	private static restServer: ExpressRestServer;
	...
	static async start(
		...
		// REST server.
		this.restServer = new ExpressRestServer(
			this.logger,
			tokenHelper,
			aggregate,
